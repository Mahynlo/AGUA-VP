...
import { construirURLImpresion, calcularEstadisticas, generarDatosMock } from '../utils/reciboUtils'
...
const useImpresionRecibos = () => {
  const { periodosInfo, siguientePeriodo, ultimoPeriodoRegistrado, ultimoPeriodoFacturado } =
    useRutas()
  ...
  const clientesConFacturasYLecturas = useMemo(() => {
    ...
    if (ciudadFiltro !== 'All') {
      list = list.filter(
        (f) => (f.cliente_ciudad || '').trim().toUpperCase() === ciudadFiltro.trim().toUpperCase()
      )
    }
  ...
  const facturasParaImprimir = useMemo(() => {
    return clientesConFacturasYLecturas.filter((factura) => clientesSeleccionados.has(factura.id))
  ...
  const handleToggleTodos = () => {
    if (clientesSeleccionados.size === recibos.length) {
      ...
      const todosSeleccionados = new Set(recibos.map((factura) => factura.id))
  ...
  const handlePruebaConDatosMock = async () => {
    const facturasMock = generarDatosMock()
    const previewUrl = await construirURLImpresion(facturasMock, true)
    ...
    try {
      ...
      console.error('Error generando datos mock:', err)
    }
  }
