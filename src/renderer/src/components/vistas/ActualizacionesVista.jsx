...
export default function ActualizacionesVista() {
  ...
  return (
    <div
      className={`min-h-[calc(100vh-4rem)] mt-16 p-4 sm:p-6 lg:p-8 bg-slate-50 dark:bg-black/20 overflow-y-auto transition-all ${
        loggedIn ? "sm:ml-24" : "max-w-5xl mx-auto"
      }`}
    >
      <div className="w-full bg-white dark:bg-zinc-950 rounded-[2rem] border border-slate-200 dark:border-zinc-800 shadow-sm p-6 sm:p-8 lg:p-10 flex flex-col gap-8 mb-12">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-6 border-b border-slate-100 dark:border-zinc-800/80">
          <div className="flex items-center gap-4">
            <button
              ...
              className="p-3 bg-slate-100 dark:bg-zinc-900 hover:bg-slate-200 dark:hover:bg-zinc-800 text-slate-700 dark:text-zinc-300 rounded-2xl transition-colors shrink-0 shadow-sm group"
              title="Volver"
            >
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm font-medium text-slate-500 dark:text-zinc-400 mt-1.5">
                Consulta novedades, descarga nuevas versiones y mantén Agua VP al día
              </p>
            </div>
          </div>

          <div className="flex items-center gap-2 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[11px] font-bold uppercase tracking-wider px-3.5 py-2 rounded-xl border border-emerald-500/20 shrink-0">
