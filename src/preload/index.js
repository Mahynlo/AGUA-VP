...
const api = {
  ...
  fetchClientesEliminados: (token_session) =>
    ipcRenderer.invoke('fetch-clientes-eliminados', token_session),
  ...
  fetchMedidoresEliminados: (token_session) =>
    ipcRenderer.invoke('fetch-medidores-eliminados', token_session),
  ...
  updateUserPermissions: (id, overrides, token) =>
    ipcRenderer.invoke('update-user-permissions', id, overrides, token),
  ...
  closeSpecificSession: (sesionId, token) =>
    ipcRenderer.invoke('close-specific-session', sesionId, token),
  closeAllUserSessions: (usuarioId, token) =>
    ipcRenderer.invoke('close-all-user-sessions', usuarioId, token),
  ...
  fetchReporteConsumoAgua: async (token_session, filtros) => {
    return await ipcRenderer.invoke('fetch-reporte-consumo-agua', token_session, filtros)
  },
  ...
  deudores: {
    ...
    updateConfiguracion: async (token, config) =>
      ipcRenderer.invoke('update-configuracion-corte', token, config),
    recalcularVencimientosPorPeriodo: async (token, data) =>
      ipcRenderer.invoke('recalcular-vencimientos-por-periodo', token, data),
    ...
    registrarReconexion: async (token, data) =>
      ipcRenderer.invoke('registrar-reconexion', token, data),
    crearConvenio: async (token, data) => ipcRenderer.invoke('crear-convenio', token, data),
    obtenerConvenio: async (token, convenioId) =>
      ipcRenderer.invoke('obtener-convenio', token, convenioId),
    pagarParcialidad: async (token, data) => ipcRenderer.invoke('pagar-parcialidad', token, data),
    fetchResumenCobroConvenio: async (token, medidorId) =>
      ipcRenderer.invoke('fetch-resumen-cobro-convenio', token, medidorId),
    pagarIntegradoConvenio: async (token, data) =>
      ipcRenderer.invoke('pagar-integrado-convenio', token, data)
  },
  ...
  clearCustomLogo: () => ipcRenderer.invoke('clear-custom-logo'),
  getDefaultLogo: () => ipcRenderer.invoke('get-default-logo'),
  ...
  system: {
    ...
    restoreBackup: (backupPath, userToken) =>
      ipcRenderer.invoke('system:backup-restore', backupPath, userToken),
...
const authApp = {
  ...
  ensureToken: (nombreApp = 'Electron App') =>
    ipcRenderer.invoke('auth-app:ensure-token', nombreApp),
  recoverOrRegister: (nombreApp = 'Electron App') =>
    ipcRenderer.invoke('auth-app:recover-or-register', nombreApp),
  registrarApp: (nombreApp = 'Electron App') =>
    ipcRenderer.invoke('auth-app:registrar-app', nombreApp),
...
const docsApp = {
  loadDocumentationFile: (section, fileName, tipo = null) =>
    ipcRenderer.invoke('load-documentation-file', section, fileName, tipo),
  loadDocumentationImage: (imagePath) => ipcRenderer.invoke('load-documentation-image', imagePath),
  listDocumentationFiles: (section = null) =>
    ipcRenderer.invoke('list-documentation-files', section),
  openHelpWindow: (section = null, file = null) =>
    ipcRenderer.invoke('open-help-window', section, file),
...
ipcRenderer.on('auth:token-expired', () => {
  console.warn(
    '🔄 [Preload] Token expirado detectado en el proceso principal. Notificando al renderer...'
  )
